...
class PGM {
   public:
    ...
    void nms();
    void doublethresh(int t1, int t2);
    void hysteresis();
    void prewitt();
    void gaussian3();
};
...
//prewitt
const int8_t Gx[] = {-1, 0, 1, -1, 0, 1, -1, 0, 1};
const int8_t Gy[] = {1, 1, 1, 0, 0, 0, -1, -1, -1};
const float G3[] = {0.0625, 0.125,  0.0625, 0.125, 0.25,
...
void PGM::prewitt() {
    ...
    for (int x = offset_xy; x < width - offset_xy; x++) {
        for (int y = offset_xy; y < height - offset_xy; y++) {
            ...
            // prewitt filter; gradient magnitude & direction
            int segment = 0;

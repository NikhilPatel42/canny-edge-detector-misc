...
void PGM::edges(int weak_threshold, int strong_threshold) {
    ...
    for (int x = 1; x < w_ - 1; x++) {
        for (int y = 1; y < h_ - 1; y++) {
            ...
            switch (tes[pos]) {
                case 1:
                    if (temp[pos - 1] >= temp[pos] || temp[pos + 1] > temp[pos]) {
                        pixel[pos] = 0;
                    }
                    break;
                case 2:
    ...
    for (int x = 1; x < w_ - 1; x++) {
        for (int y = 1; y < h_ - 1; y++) {
            int src_pos = x + (y * w_);
            if (pixel[src_pos] == 255) {
                pixel[src_pos] = 255;
            } else if (pixel[src_pos] == 100) {
                if (pixel[src_pos - 1] == 255 || pixel[src_pos + 1] == 255 ||
                    pixel[src_pos - 1 - w_] == 255 ||
                    pixel[src_pos + 1 - w_] == 255 ||
                    pixel[src_pos + w_] == 255 ||
                    pixel[src_pos + w_ - 1] == 255 ||
                    pixel[src_pos + w_ + 1] == 255) {
                    pixel[src_pos] = 255;
                } else {
                    pixel[src_pos] = 0;
